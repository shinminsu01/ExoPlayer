...
import com.google.android.exoplayer2.extractor.MetaDataRetriever;
 ...
 */
public final class TsMetaDataRetriever implements MetaDataRetriever, SeekPoint.EventListener {
  ...
  private static final int BUFFER_PACKET_COUNT = 10; // Should be at least 2
  public static final int BUFFER_SIZE = TS_PACKET_SIZE * BUFFER_PACKET_COUNT;
  ...
  private boolean finishedFeeding;

  public class DummyExtractorOutput implements ExtractorOutput {
    @Override
   ...
   */
  public TsMetaDataRetriever(@Mode int mode, TimestampAdjuster timestampAdjuster,
    ...
    output = new DummyExtractorOutput();
  ...
  public void finishFeeding() {
    durationUs = seekPoints.get(seekPoints.size() - 1).getTimeUs();
  }

  @Override
  public int feedData(byte[] data, int size) {
    tsPacketBuffer.reset(data, size);
  ...
  public ExtractorMetaData getMetaData() {
    return new ExtractorMetaData(durationUs, seekPoints);
  }

  // SeekPoint.EventListener
...
//          reader = id3Reader;
//        } else {
//          reader = payloadReaderFactory.createPayloadReader(streamType, esInfo, TsMetaDataRetriever.this);
//          if (reader != null) {
//            reader.init(timestampAdjuster, output,
//                new TrackIdGenerator(programNumber, trackId, MAX_PID_PLUS_ONE));
//          }
//        }
        if (streamType == TS_STREAM_TYPE_H264) {
          TsPayloadReader reader;
          reader = payloadReaderFactory.createPayloadReader(streamType, esInfo, TsMetaDataRetriever.this);
          if (reader != null) {
            reader.init(timestampAdjuster, output,
